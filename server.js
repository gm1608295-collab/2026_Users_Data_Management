...
// Database အသစ် Connection String
const pool = new Pool({
...
// ==================== COUNTRIES ====================
app.get('/api/countries', (req, res) => { res.json([{name:'Myanmar',regions:['Yangon','Mandalay']},{name:'Thailand',regions:['Bangkok','Chiang Mai']}]); });
...
// ==================== PAGE TOGGLE ====================
app.get('/api/admin/page_status', async (req, res) => { try { const pages = [{id:'topup',name:'Top Up'},{id:'buycode',name:'Buy Code'},{id:'dashboard',name:'Dashboard'}]; const result = []; for (const p of pages) { const r = await pool.query("SELECT status FROM page_status WHERE page_id=$1",[p.id]); result.push({id:p.id,name:p.name,status:r.rows.length>0?r.rows[0].status:'on'}); } res.json({pages:result}); } catch (e) { res.json({pages:[]}); } });
...
app.post('/api/admin/edit_user', async (req, res) => { try { await pool.query("UPDATE auth_users SET username=$1,email=$2,phone=$3 WHERE id=$4",[req.body.username,req.body.email,req.body.phone,req.body.id]); res.json({success:true}); } catch (e) { res.json({success:false}); } });
...
// ==================== ORDERS ====================
app.get('/api/admin/orders', async (req, res) => { try { const filter = req.query.filter||'all'; let q = 'SELECT * FROM orders'; const params = []; const today = new Date().toISOString().split('T')[0]; if (filter==='today'){q+=" WHERE DATE(created_at)=$1";params.push(today);} else if (filter==='yesterday'){q+=" WHERE DATE(created_at)=$1";params.push(new Date(Date.now()-86400000).toISOString().split('T')[0]);} q+=' ORDER BY id DESC'; const r = await pool.query(q,params); const tc = await pool.query("SELECT COUNT(*) FROM orders WHERE DATE(created_at)=$1",[today]); res.json({orders:r.rows,total:parseInt((await pool.query("SELECT COUNT(*) FROM orders")).rows[0].count),today:parseInt(tc.rows[0].count)}); } catch (e) { res.json({orders:[],total:0,today:0}); } });
app.post('/api/submit_order', async (req, res) => { try { const uid = parseInt(req.body.token.replace('token_','')); const u = await pool.query('SELECT username FROM auth_users WHERE id=$1',[uid]); const un = u.rows[0]?.username||'Unknown'; await pool.query('INSERT INTO orders (user_id,username,amount,payment_method,screenshot,status,submitted_user_id) VALUES ($1,$2,$3,$4,$5,$6,$7)',[uid,un,req.body.amount,req.body.payment_method,req.body.screenshot,'pending',req.body.user_id||uid]); tgSend(`🛒 New Order\n👤 ${un}\n💰 ${req.body.amount} Ks\n💳 ${req.body.payment_method}`); res.json({success:true}); } catch (e) { res.json({success:false}); } });
...
app.post('/api/admin/notice', async (req, res) => { try { await pool.query('INSERT INTO notices (message,color,created_by) VALUES ($1,$2,$3)',[req.body.message,req.body.color||'#ffffff','admin']); tgSend(`📢 ${req.body.message}`); res.json({success:true}); } catch (e) { res.json({success:false}); } });
        ...
        try {
            ...
            if (result.ok && result.result.length > 0) {
                result.result.forEach(async (update) => {
                    ...
                    if (text === '/start' || text === '/login') { sendTelegramMessage(chatId, `👋 မင်္ဂလာပါ ${firstName}!\n\nSOLO M Game Shop မှ ကြိုဆိုပါတယ်။`, mainKeyboard); }
                    else if (text === '/help') { sendTelegramMessage(chatId, `📖 Commands:\n/start /help\n\n@Solo_m28`); }
                    else { sendTelegramMessage(chatId, `အောက်ပါ ခလုတ်များကို အသုံးပြုပါ။`, mainKeyboard); }
                });
            }
...
app.get('/topup.html', async (req, res) => {
    try { const r = await pool.query("SELECT status FROM page_status WHERE page_id='topup'"); if (r.rows.length > 0 && r.rows[0].status === 'off') { return res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Maintenance</title><style>body{background:#0c0e27;color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:sans-serif;text-align:center}</style></head><body><div><h1>🚧</h1><h2 style="color:#f39c12">ယခုစာမျက်နှာကို ပြုပြင်မွမ်းမံနေပါသည်</h2><a href="/dashboard" style="color:#f39c12">Back to Dashboard</a></div></body></html>`); } } catch(e) {}
