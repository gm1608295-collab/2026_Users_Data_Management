...
async function getPool() {
    ...
    catch(e) { console.log('Ã¢Å¡ Ã¯Â¸Â DB Switch:', e.message); currentPool = pool1Active ? pool2 : pool1; pool1Active = !pool1Active; return currentPool; }
}
...
function detectDevice(ua) {
    ...
    if (browser.name) {
        deviceName += ' â€¢ ' + browser.name;
        if (browser.version) deviceName += ' ' + browser.version.split('.')[0];
    }
    if (!deviceName.trim()) deviceName = 'Unknown Device';
    
    // Device type
    if (device.type === 'mobile') { deviceType = 'Mobile'; icon = 'ðŸ“±'; isMobile = true; }
    else if (device.type === 'tablet') { deviceType = 'Tablet'; icon = 'ðŸ“±'; isMobile = true; }
    else if (/Median|WebView/i.test(ua)) { deviceType = 'App'; deviceName = 'SOLO M Game App â€¢ ' + (os.name||''); icon = 'ðŸŽ®'; isMobile = true; }
...
async function trackLogin(userId, username, loginType, req) {
    try {
        ...
        // âœ… Save device session - use user_id + device fingerprint as unique key
...
function sendOnesignal(msg, sound, title) { 
    try {
        // âœ… Set default if not provided
        ...
        console.log('[ONESIGNAL] ðŸ“² Sending:', {
        ...
        const req = https.request({ 
            ...
            res.on('end', () => {
                ...
                console.log('[ONESIGNAL] ðŸ“² Push sent with sound:', notificationSound);
            });
        });
...
async function createSpinHistoryV2Table() {
    ...
    try {
        ...
        console.log('âœ… spin_history_v2 table created on both databases');
    } catch(e) {
        console.log('âš ï¸ spin_history_v2 table error:', e.message);
    }
}
...
const ALL_PAGES = [
    ...
    { id: 'game', name: 'Lucky Spin' },        // âœ… á€‘á€Šá€·á€ºá€•á€«
    { id: 'exchange', name: 'Exchange' }         // âœ… á€‘á€Šá€·á€ºá€•á€«
];
...
app.post('/api/upload_music', async (req, res) => {
    ...
    try {
        ...
        if (isVideo && !isM4A) return res.json({ success: false, message: 'Ã¢ÂÅ’ Video not allowed! Audio only.' });
        if (!isAudio && !isVideo) return res.json({ success: false, message: 'Ã¢ÂÅ’ Invalid file!' });
        let base64Data;
...
app.post('/api/admin/ban', async (req, res) => { try { const p = await getPool(); await p.query('INSERT INTO banned_users (user_id,banned_by) VALUES ($1,$2) ON CONFLICT (user_id) DO UPDATE SET banned_by=$2', [req.body.userId, 'admin']); tgSend('Ã°Å¸Å¡Â« Banned: ' + req.body.userId); res.json({ success: true }); } catch(e) { res.json({ success: false }); } });
...
app.post('/api/admin/delete', async (req, res) => { 
    try { 
        ...
        tgSend(`Ã°Å¸â€”â€˜Ã¯Â¸Â Deleted: ${userId}`);
        res.json({ success: true }); 
    } catch(e) { 
        res.json({ success: false }); 
    } 
...
app.post('/api/admin/bot_message', async (req, res) => {
    ...
    try {
        ...
        // 2. âœ… OneSignal Push Notification (with selected sound)
        ...
        console.log('[BOT MESSAGE] âœ… Sent - Telegram: ' + telegramCount + ', Push: yes, Sound: ' + selectedSound);
...
// ==================== TELEGRAM BOT (á€™á€¼á€”á€ºá€™á€¬á€œá€­á€¯) ====================
let lastUpdateId = 0;
...
async function createTelegramUser(userId, firstName) {
    try {
        ...
        // âœ… Track login for new user
        trackTelegramLogin(nu.rows[0].id, displayName);
        
        tgSend(`ðŸ†• Telegram User á€¡á€žá€…á€º\nðŸ‘¤ ${displayName}\nðŸ†” ${tgId}`);
...
function startLongPolling() {
    console.log('ðŸ¤– Telegram Bot á€…á€á€„á€ºá€•á€«á€•á€¼á€®');
    
    const mainKeyboard = {
        inline_keyboard: [
            [{ text: 'ðŸ  á€¡á€€á€±á€¬á€„á€·á€ºá€á€„á€ºá€›á€”á€º', url: 'https://two026-users-data-management.onrender.com' }],
            [{ text: 'ðŸ’° á€„á€½á€±á€–á€¼á€Šá€·á€ºá€›á€”á€º', url: 'https://two026-users-data-management.onrender.com/topup.html' }],
            [{ text: 'ðŸ›’ Code á€á€šá€ºá€›á€”á€º', url: 'https://two026-users-data-management.onrender.com/buycode.html' }],
            [{ text: 'ðŸ“ž Admin á€†á€€á€ºá€žá€½á€šá€ºá€›á€”á€º', url: 'https://t.me/Solo_m28' }]
        ]
    };
    
    const quickKeyboard = {
        inline_keyboard: [
            [{ text: 'ðŸ’³ á€œá€€á€ºá€€á€»á€”á€ºá€€á€¼á€Šá€·á€ºá€›á€”á€º', callback_data: 'balance' }],
            [{ text: 'ðŸ” OTP á€›á€šá€°á€›á€”á€º', callback_data: 'otp' }],
            [{ text: 'ðŸ“‹ Order á€…á€…á€ºá€›á€”á€º', callback_data: 'status' }],
            [{ text: 'ðŸ›’ Code á€á€šá€ºá€›á€”á€º', callback_data: 'buycode' }],
            [{ text: 'ðŸ“ž á€†á€€á€ºá€žá€½á€šá€ºá€›á€”á€º', url: 'https://t.me/Solo_m28' }]
        ]
    };
        ...
        try {
            ...
            if (result.ok && result.result.length > 0) {
                for (const update of result.result) {
                    ...
                    // ========== á€á€œá€¯á€á€ºá€”á€¾á€­á€•á€ºá€á€¼á€„á€ºá€¸á€™á€»á€¬á€¸ ==========
                    if (update.callback_query) {
                        ...
                        if (!user) { sendTelegramMessage(chatId, 'âŒ Error á€›á€¾á€­á€”á€±á€•á€«á€žá€Šá€ºá‹ /start á€€á€­á€¯á€”á€¾á€­á€•á€ºá€•á€«'); continue; }
                        
                        if (data === 'balance') {
                            const balance = user.balance || 0;
                            sendTelegramMessage(chatId, 
                                `ðŸ’³ <b>á€žá€„á€·á€ºá€œá€€á€ºá€€á€»á€”á€º</b>\n\n` +
                                `ðŸ’° <b>${balance.toLocaleString()} á€€á€»á€•á€º</b>\n` +
                                `ðŸ’µ â‰ˆ $${(balance/2100).toFixed(2)} USD\n\n` +
                                `á€„á€½á€±á€–á€¼á€Šá€·á€ºá€›á€”á€º Top Up á€á€œá€¯á€á€ºá€€á€­á€¯á€”á€¾á€­á€•á€ºá€•á€«á‹`, 
                                quickKeyboard
                            );
                        }
                        else if (data === 'otp') {
                            ...
                            sendTelegramMessage(chatId, 
                                `ðŸ” <b>á€žá€„á€·á€º OTP á€€á€¯á€’á€º</b>\n\n` +
                                `ðŸ”¢ <b>${otp}</b>\n\n` +
                                `â° á†á€ á€…á€€á€¹á€€á€”á€·á€ºá€¡á€á€½á€„á€ºá€¸ á€¡á€žá€¯á€¶á€¸á€•á€¼á€¯á€•á€«á‹\n` +
                                `âš ï¸ á€™á€Šá€ºá€žá€°á€·á€€á€­á€¯á€™á€»á€¾ á€™á€•á€±á€¸á€•á€«á€”á€¾á€„á€·á€ºá‹`, 
                                quickKeyboard
                            );
                        }
                        else if (data === 'status') {
                            ...
                            if (orders.length === 0) {
                                sendTelegramMessage(chatId, 'ðŸ“‹ á€™á€¾á€¬á€šá€°á€™á€¾á€¯á€™á€›á€¾á€­á€žá€±á€¸á€•á€«á‹', quickKeyboard);
                            } else {
                                let msg = 'ðŸ“‹ <b>á€”á€±á€¬á€€á€ºá€†á€¯á€¶á€¸ á€™á€¾á€¬á€šá€°á€™á€¾á€¯á€™á€»á€¬á€¸</b>\n\n';
                                orders.forEach(o => {
                    ...
                    // ========== á€…á€¬á€á€­á€¯á€•á€±á€¸á€•á€­á€¯á€·á€á€¼á€„á€ºá€¸á€™á€»á€¬á€¸ ==========
                    ...
                    if (text === '/start' || text === '/login') {
                        ...
                        const welcomeMsg = user.isNew ? 
                            `ðŸŽ‰ <b>SOLO M Game Shop á€™á€¾ á€€á€¼á€­á€¯á€†á€­á€¯á€•á€«á€á€šá€º!</b>\n\ná€™á€„á€ºá€¹á€‚á€œá€¬á€•á€« ${firstName}!\n\ná€žá€„á€·á€ºá€¡á€€á€±á€¬á€„á€·á€ºá€€á€­á€¯ á€¡á€œá€­á€¯á€¡á€œá€»á€±á€¬á€€á€º á€–á€½á€„á€·á€ºá€•á€±á€¸á€•á€¼á€®á€¸á€•á€«á€•á€¼á€®á‹` :
                            `ðŸ‘‹ á€•á€¼á€”á€ºá€œá€Šá€ºá€€á€¼á€­á€¯á€†á€­á€¯á€•á€«á€á€šá€º ${firstName}!`;
                        
                        sendTelegramMessage(chatId, 
                            welcomeMsg + '\n\n' +
                            'ðŸ’³ á€œá€€á€ºá€€á€»á€”á€º: <b>' + (user.balance || 0).toLocaleString() + ' á€€á€»á€•á€º</b>\n\n' +
                            'á€¡á€±á€¬á€€á€ºá€•á€« á€á€œá€¯á€á€ºá€™á€»á€¬á€¸á€€á€­á€¯ á€¡á€žá€¯á€¶á€¸á€•á€¼á€¯á€”á€­á€¯á€„á€ºá€•á€«á€žá€Šá€ºá‹',
                            quickKeyboard
                        );
                    }
                    else if (text === '/help') {
                        sendTelegramMessage(chatId,
                            ...
                            `<b>Commands á€™á€»á€¬á€¸:</b>\n` +
                            `/start - á€¡á€€á€±á€¬á€„á€·á€ºá€á€„á€ºá€›á€”á€º\n` +
                            `/help - á€¡á€€á€°á€¡á€Šá€®\n` +
                            `/balance - á€œá€€á€ºá€€á€»á€”á€ºá€€á€¼á€Šá€·á€ºá€›á€”á€º\n` +
                            `/otp - OTP Code\n` +
                            `/status - Order á€…á€…á€ºá€›á€”á€º\n` +
                            `/buy - Code á€á€šá€ºá€›á€”á€º\n\n` +
                        ...
                        if (orders.length === 0) {
                            sendTelegramMessage(chatId, 'ðŸ“‹ á€™á€¾á€¬á€šá€°á€™á€¾á€¯á€™á€›á€¾á€­á€žá€±á€¸á€•á€«á‹');
                        } else {
                            let msg = 'ðŸ“‹ <b>á€”á€±á€¬á€€á€ºá€†á€¯á€¶á€¸ á€™á€¾á€¬á€šá€°á€™á€¾á€¯á€™á€»á€¬á€¸</b>\n\n';
                            orders.forEach(o => {
                                const st = o.status === 'approved' ? 'âœ…' : o.status === 'rejected' ? 'âŒ' : 'â³';
...
function maintenancePage() {
    return `<!DOCTYPE html><html lang="my"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no"><title>Ã¡â‚¬â€¢Ã¡â‚¬Â¼Ã¡â‚¬Â¯Ã¡â‚¬â€¢Ã¡â‚¬Â¼Ã¡â‚¬â€žÃ¡â‚¬ÂºÃ¡â‚¬â„¢Ã¡â‚¬Â½Ã¡â‚¬â„¢Ã¡â‚¬ÂºÃ¡â‚¬Â¸Ã¡â‚¬â„¢Ã¡â‚¬Â¶Ã¡â‚¬â€Ã¡â‚¬Â±Ã¡â‚¬â€¢Ã¡â‚¬Â«Ã¡â‚¬Å¾Ã¡â‚¬Å Ã¡â‚¬Âº</title><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"><style>*{margin:0;padding:0}body{background:linear-gradient(135deg,#0c0e27,#1a1f4b,#2c3e50);min-height:100vh;display:flex;justify-content:center;align-items:center;text-align:center;font-family:sans-serif;color:#fff}.box i{font-size:70px;color:#f39c12;animation:pulse 2s infinite}@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}.box h2{color:#f39c12;margin:15px 0}.box p{color:#ccc;margin-bottom:20px}.box a{color:#000;background:#f39c12;padding:10px 25px;border-radius:6px;text-decoration:none;font-weight:bold}</style></head><body><div class="box"><i class="fas fa-tools"></i><h2>Ã¡â‚¬Å¡Ã¡â‚¬ÂÃ¡â‚¬Â¯Ã¡â‚¬â€¦Ã¡â‚¬Â¬Ã¡â‚¬â„¢Ã¡â‚¬Â»Ã¡â‚¬â‚¬Ã¡â‚¬ÂºÃ¡â‚¬â€Ã¡â‚¬Â¾Ã¡â‚¬Â¬Ã¡â‚¬â‚¬Ã¡â‚¬Â­Ã¡â‚¬Â¯ Ã¡â‚¬â€¢Ã¡â‚¬Â¼Ã¡â‚¬Â¯Ã¡â‚¬â€¢Ã¡â‚¬Â¼Ã¡â‚¬â€žÃ¡â‚¬ÂºÃ¡â‚¬â„¢Ã¡â‚¬Â½Ã¡â‚¬â„¢Ã¡â‚¬ÂºÃ¡â‚¬Â¸Ã¡â‚¬â„¢Ã¡â‚¬Â¶Ã¡â‚¬â€Ã¡â‚¬Â±Ã¡â‚¬â€¢Ã¡â‚¬Â«Ã¡â‚¬Å¾Ã¡â‚¬Å Ã¡â‚¬Âº</h2><p>Ã¡â‚¬â‚¬Ã¡â‚¬Â»Ã¡â‚¬Â±Ã¡â‚¬Â¸Ã¡â‚¬â€¡Ã¡â‚¬Â°Ã¡â‚¬Â¸Ã¡â‚¬â€¢Ã¡â‚¬Â¼Ã¡â‚¬Â¯Ã¡ÂÂ Ã¡â‚¬ÂÃ¡â‚¬ÂÃ¡â‚¬â€¦Ã¡â‚¬Â±Ã¡â‚¬Â¬Ã¡â‚¬â€žÃ¡â‚¬Â·Ã¡â‚¬ÂºÃ¡â‚¬â€ Ã¡â‚¬Â­Ã¡â‚¬Â¯Ã¡â‚¬â€žÃ¡â‚¬ÂºÃ¡â‚¬Â¸Ã¡â‚¬â€¢Ã¡â‚¬Â±Ã¡â‚¬Â¸Ã¡â‚¬â€¢Ã¡â‚¬Â«Ã¡Ââ€¹</p><a href="/dashboard"><i class="fas fa-arrow-left"></i> Ã¡â‚¬â€¢Ã¡â‚¬â€žÃ¡â‚¬ÂºÃ¡â‚¬â„¢Ã¡â‚¬â€¦Ã¡â‚¬Â¬Ã¡â‚¬â„¢Ã¡â‚¬Â»Ã¡â‚¬â‚¬Ã¡â‚¬ÂºÃ¡â‚¬â€Ã¡â‚¬Â¾Ã¡â‚¬Â¬Ã¡â‚¬Å¾Ã¡â‚¬Â­Ã¡â‚¬Â¯Ã¡â‚¬Â·</a></div></body></html>`;
}
...
// â•‘              SPIN & USD API (COMPLETE REWRITE)              â•‘
// â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

// ==================== SPIN CONFIG (SERVER-SIDE) ====================
const SPIN_CONFIG = {
    // âœ… Normal Daily 1 Draw (Total Weight = 101)
    NORMAL_SEGMENTS: [
    ...
    // âœ… Buy Normal Draw (Total Weight = 103)
    NORMAL_BUY_SEGMENTS: [
    ...
    // âœ… Buy Premium Draw (Total Weight = 94)
    ...
    // âœ… Premium Tier 1 - Bronze (Total Weight = 101)
    ...
    // âœ… Premium Tier 2 - Silver (Total Weight = 98)
    ...
    // âœ… Premium Tier 3 - Gold (Total Weight = 102)
...
app.post('/api/deduct_balance', async (req, res) => {
    ...
    try {
        ...
        if (balance < amount) return res.json({ success: false, message: 'á€„á€½á€±á€™á€œá€¯á€¶á€œá€±á€¬á€€á€ºá€•á€«á‹ Top Up á€œá€¯á€•á€ºá€•á€«á‹' });
...
app.post('/api/buy_premium', async (req, res) => {
    ...
    try {
        ...
        if (balance < cost) return res.json({ success: false, message: 'á€„á€½á€±á€™á€œá€¯á€¶á€œá€±á€¬á€€á€ºá€•á€«á‹ Top Up á€œá€¯á€•á€ºá€•á€«á‹' });
...
// â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—
// â•‘           SPIN EXECUTE API (MAIN SPIN LOGIC)                â•‘
// â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
app.post('/api/spin/execute', async (req, res) => {
    ...
    try {
        ...
        // âœ… Premium Tier á€¡á€œá€­á€¯á€€á€º Max Daily
        let maxDaily = 1;
        if (isPremium) {
            switch(premiumTier) { case 1: maxDaily = 3; break; case 2: maxDaily = 5; break; case 3: maxDaily = 7; break; default: maxDaily = 3; }
        }
        ...
        // âœ… Weighted Random - Source á€¡á€œá€­á€¯á€€á€º Segments á€›á€½á€±á€¸á€•á€«
        let segments;
        if (!isPremium) {
        ...
        console.log('[SPIN RESULT] âœ… User:', uid, 'Tier:', premiumTier, 'Reward:', reward.label);
...
console.log('âœ… SPIN & USD API Section Ready');
...
async function createSpinRatesTable() {
    ...
    try {
        await pool1.query(query);
        await pool2.query(query);
        console.log('âœ… spin_rates table created on both databases');
    } catch(e) {
        console.log('âš ï¸ spin_rates table error:', e.message);
    }
}
...
async function initDefaultSpinRates() {
    ...
    try {
        ...
        console.log('âœ… Default spin rates initialized');
    } catch(e) {
        console.log('âš ï¸ Default spin rates error:', e.message);
    }
}
...
app.post('/api/admin/spin_rates/save', async (req, res) => {
    ...
    try {
        ...
        console.log('[SPIN RATES] âœ… Updated by admin');
...
async function createPromoCodesTable() {
    ...
    try {
        await pool1.query(query);
        await pool2.query(query);
        console.log('âœ… promo_codes table ready');
    } catch(e) {
        console.log('âš ï¸ promo_codes table error:', e.message);
    }
}
...
app.post('/api/buy_promo_code', async (req, res) => {
    ...
    try {
        ...
        // âœ… Don't mark as used yet! User must redeem first
        // await p.query('UPDATE promo_codes SET used = true, used_by = $1, used_at = NOW() WHERE id = $2', [uid, c.id]);
        
        // âœ… Just deduct balance and mark as SOLD (but not redeemed)
...
app.post('/api/redeem_promo', async (req, res) => {
    ...
    try {
        ...
        // âœ… 1. Check if code exists and is SOLD (used=true) but NOT REDEEMED (used_by IS NULL)
        const code = await p.query(
        ...
        if (code.rows.length > 0) {
            // âœ… Code found - user can redeem it (one time only)
            const c = code.rows[0];
            ...
            return res.json({
                success: true,
                message: `âœ… á€¡á€±á€¬á€„á€ºá€™á€¼á€„á€ºá€•á€«á€žá€Šá€º! ${c.amount.toLocaleString()} ${c.currency} á€›á€›á€¾á€­á€•á€«á€•á€¼á€®`,
                amount: c.amount,
                currency: c.currency,
        ...
        // âœ… 2. Check if code is already redeemed (used=true AND used_by IS NOT NULL)
        ...
        if (redeemedCheck.rows.length > 0) {
            return res.json({ success: false, message: 'Code á€€á€­á€¯á€¡á€žá€¯á€¶á€¸á€•á€¼á€¯á€•á€¼á€®á€¸á€•á€«á€•á€¼á€® (á€á€…á€ºá€€á€¼á€­á€™á€ºá€•á€²á€žá€¯á€¶á€¸á€œá€­á€¯á€·á€›á€•á€«á€žá€Šá€º)' });
        }
        
        // âœ… 3. Check if code exists but not sold yet
        ...
        if (unsoldCheck.rows.length > 0) {
            return res.json({ success: false, message: 'Code á€™á€¾á€¬á€¸á€šá€½á€„á€ºá€¸á€”á€±á€•á€«á€žá€Šá€º' });
        }
        ...
        if (expiredCheck.rows.length > 0) {
            return res.json({ success: false, message: 'Code á€žá€€á€ºá€á€™á€ºá€¸á€€á€¯á€”á€ºá€žá€½á€¬á€¸á€•á€«á€•á€¼á€®' });
        }
...
console.log('âœ… Promo Code System Ready');
...
app.post('/api/admin/checkin_event/edit', async (req, res) => {
    ...
    try {
        ...
        if (eventStart <= now) {
            return res.json({ success: false, message: 'á€…á€á€„á€ºá€•á€¼á€®á€¸á€žá€¬á€¸ Event á€€á€­á€¯ á€•á€¼á€„á€ºá€†á€„á€ºáá€™á€›á€•á€«' });
        }
...
app.post('/api/admin/checkin_event/cancel', async (req, res) => {
    ...
    try {
        ...
        if (eventStart <= now) {
            return res.json({ success: false, message: 'á€…á€á€„á€ºá€•á€¼á€®á€¸á€žá€¬á€¸ Event á€€á€­á€¯ á€–á€»á€€á€ºáá€™á€›á€•á€«' });
        }
...
app.post('/api/daily_checkin/status', async (req, res) => {
    ...
    start_date: event.start_date,  // âœ… á€’á€®á€…á€¬á€€á€¼á€±á€¬á€„á€ºá€¸ á€‘á€Šá€·á€ºá€•á€«
...
async function initDefaultCheckinEvents() {
    ...
    while (retries > 0) {
        try {
            ...
            if (parseInt(existing.rows[0].count) > 0) {
                console.log('âœ… Check-in events already exist');
                return;
            }
            tableReady = true;
            break;
            ...
            retries--;
            console.log('â³ Waiting for tables... (' + retries + ' retries left)');
            await new Promise(function(r) { setTimeout(r, 2000); });
        }
    }
    
    if (!tableReady) {
        console.log('âš ï¸ Could not verify tables, skipping default events');
        return;
    }
    ...
    try {
        ...
        var normalRewards = [
            { day: 1, type: 'spin', amount: 1, label: '1 Draw Spin', icon: 'ðŸŽ' },
            { day: 2, type: 'mmk', amount: 100, label: '100 Ks', icon: 'ðŸ’°' },
            { day: 3, type: 'mmk', amount: 150, label: '150 Ks', icon: 'ðŸ’°' },
            { day: 4, type: 'usd', amount: 0.05, label: '$0.05 USD', icon: 'ðŸ’µ' },
            { day: 5, type: 'spin', amount: 1, label: '1 Draw Spin', icon: 'ðŸ”„' },
            { day: 6, type: 'usd', amount: 0.10, label: '$0.10 USD', icon: 'ðŸ’µ' },
            { day: 7, type: 'mmk', amount: 300, label: '300 Ks', icon: 'ðŸŽ‰' }
        ];
        ...
        console.log('âœ… Normal check-in event created');
        ...
        var premRewards = [
            { day: 1, type: 'mmk', amount: 200, label: '200 Ks', icon: 'ðŸ’°' },
            { day: 2, type: 'spin', amount: 1, label: '1 Draw Spin', icon: 'ðŸ”„' },
            { day: 3, type: 'usd', amount: 0.15, label: '$0.15 USD', icon: 'ðŸ’µ' },
            { day: 4, type: 'spin', amount: 2, label: '2 Draw Spins', icon: 'ðŸ”„' },
            { day: 5, type: 'mmk', amount: 300, label: '300 Ks', icon: 'ðŸ’°' },
            { day: 6, type: 'mmk', amount: 350, label: '350 Ks', icon: 'ðŸ’°' },
            { day: 7, type: 'usd', amount: 0.15, label: '$0.15 USD', icon: 'ðŸ’µ' },
            { day: 8, type: 'spin', amount: 2, label: '2 Draw Spins', icon: 'ðŸ”„' },
            { day: 9, type: 'spin', amount: 2, label: '2 Draw Spins', icon: 'ðŸ”„' },
            { day: 10, type: 'spin', amount: 2, label: '2 Draw Spins', icon: 'ðŸ”„' },
            { day: 11, type: 'mmk', amount: 400, label: '400 Ks', icon: 'ðŸ’°' },
            { day: 12, type: 'usd', amount: 0.20, label: '$0.20 USD', icon: 'ðŸ’µ' },
            { day: 13, type: 'mmk', amount: 500, label: '500 Ks', icon: 'ðŸ’°' },
            { day: 14, type: 'usd', amount: 0.25, label: '$0.25 USD', icon: 'ðŸ’µ' }
        ];
        ...
        console.log('âœ… Premium check-in event created');
        console.log('âœ… Default check-in events created on both databases');
        
    } catch(e) {
        console.log('âš ï¸ Default check-in events error:', e.message);
    }
}

// âŒ Default Events á€™á€–á€”á€ºá€á€®á€¸á€á€»á€„á€ºá€›á€„á€º á€’á€® áƒ á€€á€¼á€±á€¬á€„á€ºá€¸á€€á€­á€¯ comment á€œá€¯á€•á€ºá€‘á€¬á€¸á€•á€«
// setTimeout(function() {
    ...
    try {
        ...
        const r = await p.query(`
            SELECT 
                ...
                (SELECT CONCAT(COALESCE(device_brand,''), ' ', COALESCE(device_model,''), ' â€¢ ', COALESCE(browser,'')) FROM login_history lh WHERE lh.user_id = u.id AND device_brand IS NOT NULL ORDER BY lh.login_at DESC LIMIT 1) as last_device,
...
async function createResellerTables() {
    ...
    try {
        ...
        console.log('âœ… Reseller tables ready');
        ...
        console.log('âš ï¸ Reseller tables error:', e.message);
    }
}
...
console.log('âœ… API Key & Reseller System Ready');
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`Server running on port ${PORT}`);     // âœ… Server á€…á€•á€¼á€®
    console.log(`DB: DB1 + DB2 Auto-Switch`);           // á€’á€±á€á€¬á€˜á€±á€·á€…á€º á‚á€á€¯
    console.log(`Page Control: ${ALL_PAGES.length} pages`); // á€…á€¬á€™á€»á€€á€ºá€”á€¾á€¬ áá á€á€¯
    console.log(`Bot: Enhanced Long Polling`);           // Telegram Bot
    console.log(`Redeem Codes: ${REDEEM_CATEGORIES.length} categories`); // Code á… á€™á€»á€­á€¯á€¸
});
