...
async function createTelegramUser(userId, firstName, username) {
    try {
        ...
        if (exist.rows.length > 0) {
            ...
            return { id: exist.rows[0].id, username: exist.rows[0].username, email: exist.rows[0].email, balance: exist.rows[0].balance };
        }
        ...
        return { id: newUser.rows[0].id, username: firstName || username, email: 'tg_' + userId + '@telegram.com', balance: 0 };
    } catch(e) { return null; }
}
...
app.post('/api/request_otp', async (req, res) => {
    ...
    try {
        ...
        await pool.query('UPDATE otp_codes SET used = true WHERE user_id = $1 AND used = false', [uid]);
        await pool.query('INSERT INTO otp_codes (user_id, code, expires_at) VALUES ($1, $2, NOW() + INTERVAL \'5 minutes\')', [uid, otp]);
        const user = await pool.query('SELECT username FROM auth_users WHERE id = $1', [uid]);
        const un = user.rows.length > 0 ? user.rows[0].username : 'User';
        tgSend(`🔐 OTP Code\n👤 ${un}\n🔢 ${otp}\n⏰ 5 minutes valid`);
...
app.post('/api/admin/order_status', async (req, res) => { const { id, status } = req.body; try { const order = await pool.query('SELECT * FROM orders WHERE id = $1', [id]); const o = order.rows[0]; await pool.query('UPDATE orders SET status=$1 WHERE id=$2', [status, id]); if (status === 'approved') { tgSend(`✅ Order Approved\n👤 ${o.username}\n💰 ${o.amount} Ks\n💳 ${o.payment_method}`); } else if (status === 'rejected') { tgSend(`❌ Order Rejected\n👤 ${o.username}\n💰 ${o.amount} Ks\n💳 ${o.payment_method}`); } res.json({ success: true }); } catch (e) { res.json({ success: false }); } });
...
// ==================== NOTICE ====================
app.get('/api/notice', async (req, res) => { try { const r = await pool.query('SELECT * FROM notices ORDER BY id DESC LIMIT 1'); if (r.rows.length === 0) return res.json({ success: true, message: '', color: '#ffffff', image_url: '' }); const n = r.rows[0]; res.json({ success: true, message: n.message, color: n.color || '#ffffff', id: n.id, created_at: n.created_at, image_url: n.image_url || '' }); } catch (e) { res.json({ success: true, message: '', image_url: '' }); } });
...
app.post('/api/admin/notice', async (req, res) => { const { message, color, image_url } = req.body; if (!message) return res.json({ success: false }); try { await pool.query('INSERT INTO notices (message, color, image_url, created_by) VALUES ($1,$2,$3,$4)', [message, color || '#ffffff', image_url || '', 'admin']); tgSend(`📢 ${message}`); sendOnesignal(message); res.json({ success: true }); } catch (e) { res.json({ success: false }); } });
...
app.post('/telegram-webhook', async (req, res) => {
    ...
    if (text === '/start' || text === '/login') {
        ...
        const otp = Math.floor(100000 + Math.random() * 900000).toString();
        await pool.query('UPDATE otp_codes SET used = true WHERE user_id = $1 AND used = false', [user.id]);
        await pool.query('INSERT INTO otp_codes (user_id, code, expires_at) VALUES ($1, $2, NOW() + INTERVAL \'5 minutes\')', [user.id, otp]);
        sendTelegramMessage(chatId, `🔐 သင်၏ OTP Code\n\n🔢 <b>${otp}</b>\n\n⏰ ၅ မိနစ်အတွင်း အသုံးပြုပါ။\n\nဤ OTP ကို မည်သူ့ကိုမျှ မပေးပါနှင့်။`);
    }
    else {
